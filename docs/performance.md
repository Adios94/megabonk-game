...
**机制**：
- Pass 1：整场景渲到 HalfFloat RT + 深度纹理
- Pass 2：全屏 shader 深度边缘描边合成
- 之后还有 `OutputPass`（tone map）+ `ColorGradePass`（调色）
- 等效 **2× 场景几何 + 2× 全屏后处理**（Bloom 关掉后仍是最大头）

**已有开关**：`outlinePass.mode = 'none'` 时只 blit 颜色（dev `O` 键 A/B）。

**注意**：`OutputPass` 在 `ColorGradePass` 之前，多了一次全屏 blit + 一次 sample，可考虑合并 shader。
...
| 2.2 | 移动端 `outlinePass.mode='none'`，进一步可在 `renderFrame()` 跳过 EffectComposer，直接 `renderer.render(scene, camera)`（视觉损失：无描边、无调色） | `setupComposer()` / `renderFrame()` `index.ts:4518-4559` | 省 1 整 3D pass + 2 全屏 pass |
...
| # | 改动 | 文件:行号 |
|---|---|---|
| 3.1 | Balanced/Performance 档自动 `outlinePass.mode='none'`（沿用 dev `O` 键已有逻辑） | `setupComposer()` |
...
| 3.4 | `EffectComposer` 改 `{ type: THREE.UnsignedByteType }`（bloom 关后 HalfFloat 无收益） | `index.ts:4520` |
| 3.5 | （可选）合并 ColorGrade 进 OutputPass | `index.ts:4544-4549` |
...
| 档位 | dpr 上限 | Outline | Shadow | 弯曲世界 | 地面 tessellate | 后处理 | 目标用户 |
|---|---:|---|---|---|---|---|---|
| **Ultra** | 2 | screenSpace | 2048 PCFSoft | 开 | 1.8 | 全 | 独显 / 高刷 |
| **High**（默认） | 2 | screenSpace | 2048 PCFSoft | 开 | 1.8 | 全 | 当前桌面基线 |
| **Balanced** | 动态 1~1.5 | none | 1024 Basic | 开 | 1.8 | OutputPass + ColorGrade | 集显 / 4K 笔记本 |
| **Performance** | 1 | none | 512 或关 | 关 | 4 | 仅 OutputPass | 录屏 / 低配 |
| **Mobile** | 1 | none | 关或 512 | 关 | 4 | 跳过 Composer，直接 `renderer.render` | iPhone / Android |
