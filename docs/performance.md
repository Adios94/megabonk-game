...
`SpatialHash.queryRef` (`game/core/source/spatialHash.ts:86-89`) 是已写好的零分配版本，但只在 `collisions.ts`（投射物→敌人）一处用上。其余 5 处全是裸 O(N) 扫敌人：
...
| ~~3.3~~ | ~~动态 DPR 反馈环~~ —— **降级为🟢可选**，详见下方说明 | — |

> §4.5 第一刀的 `getQualityTier()` 启动 5s benchmark 已覆盖低配设备的自动降档（dpr 在档位级别钉死）；OutputPass / ColorGrade / DarkComic / sceneRT 类型变更均已并入 §4.5 FinalCompositePass 重构，不再单列。late_combat 退化的根因是 sim tick CPU（§3.2 / 阶段 4），不是填充率，运行时调 DPR 治标不治本且会带来视觉抖动。3.3 推到可选：仅当阶段 4 做完后 4K 外接屏 / Boss 战仍有 `render submit > 14 ms` 实测时再启用。
...
- [ ] **阶段 3.1-3.2** 桌面 OutlinePass 分级 + Shadow 分级
...
- [ ] **阶段 3.3** 动态 DPR 反馈环（仅当阶段 4 做完后 4K / Boss 仍超 14ms 才启用）
...
| 4K 外接屏全屏 | Mac Chrome | dev overlay `render submit` | <14 ms（阶段 3.1-3.2 后；如仍超阈值则启用阶段 3.3 动态 dpr） |
...
| Spatial Hash | `game/core/source/spatialHash.ts` |
...
| 2026-06-21 | v1.2：阶段 3 表收敛——3.4 / 3.5 删除（已并入 §4.5）；3.3 动态 DPR 反馈环由🔴必做降为🟢可选，仅当阶段 4 sim tick 优化完成后 4K / Boss 仍触发 14ms 阈值才启用（理由：late_combat 退化根因是 CPU sim tick，启动 benchmark 已覆盖低配自动降档，运行时动态 dpr 视觉抖动负收益）；§5 优先级清单 / §7 测试矩阵同步 |
