...
func _describe_option(opt: Dictionary) -> String:
	match opt["kind"]:
		"new_weapon":
			return "%s\n[新武器]\n%s" % [_i18n_weapon(opt["id"]), _rarity_tag(opt["rarity"])]
		"weapon_upgrade":
			var cur := _get_weapon_level(opt["id"])
			return "%s\n[Lv %d → %d]\n%s" % [_i18n_weapon(opt["id"]), cur, cur + 1, _rarity_tag(opt["rarity"])]
		"tome":
			var cur := _get_tome_level(opt["id"])
			return "%s\n[Lv %d → %d]\n%s" % [_i18n_tome(opt["id"]), cur, cur + 1, _rarity_tag(opt["rarity"])]
	return str(opt)


func _rarity_tag(r: String) -> String:
	return "★" if r == "legendary" else ("◆" if r == "rare" else "•")


func _i18n_weapon(id: String) -> String:
	# 尝试 weapon.<id>.name，没找到就 fallback 到 id
	var name: String = I18n.t("weapon.%s.name" % id)
	if name.begins_with("weapon."):
...
func _i18n_tome(id: String) -> String:
	var name: String = I18n.t("weapon.tome.%s" % id)
	if name.begins_with("weapon.tome."):
		return id.replace("_", " ").capitalize()
	return name
