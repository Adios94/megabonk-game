...
| 传送门 / 飞碟 | turret_teleporter.gltf        | 加载 index.ts:1684；摆放 `renderTeleporters()` index.ts:5334        | OK（缺模型时 fallback 蓝环+光柱） |
...
> 已贴图化，见 §4 与 §9；飞碟地面魔法阵（magic_circle.png）、宝箱模型（Prop_Chest gltf）已就位。
...
| index.ts:5701 | 飞碟 fallback 圆环 | RingGeometry(1.5, 2.0)            | 飞碟圆环贴花            | P1  |
| index.ts:5716 | 飞碟光柱           | CylinderGeometry(0.3, 1.5, 4)     | 光柱 sprite         | P1  |
...
| magic_circle | textures/vfx/magic_circle.png | 飞碟 / 法阵                                       |
...
5. 神坛 / 飞碟 圆环替换为带美术的贴花（Boss 攻击预警圆环已移除）
6. 宝箱光柱 / 神坛光柱 sprite
